...
All figures below are in `outputs/figures/`. The six numbered ones are
generated by `scripts/evaluate.py` from a real 5-fold cross-validation run
(see `outputs/logs/evaluation_results.json` for the underlying numbers);
the pipeline diagram is a standalone architecture illustration, not a
generated evaluation output.

<!-- PLACEMENT: default order below (pipeline first, then figures 1-5b) --
     tell me if you want a different order/grouping and I'll rearrange. -->

### Pipeline architecture

![Overall system architecture](outputs/figures/figure0_pipeline_architecture.png)

*Figure 0: Overall system architecture — sensor recording flows through
preprocessing, the (non-LLM) recognition and aggregation layers, and an
interface layer that uses Ollama (qwen3:8b) only to parse the question and
phrase the final explanation.*

### Figure 1 — Accuracy by question type

![Accuracy by question type](outputs/figures/figure1_accuracy_by_question_type.png)

*Figure 1: End-to-end QA accuracy broken down by question type. Comparison
questions score highest (0.84); duration (0.12) and count (0.04) are the
weakest, since they depend on exact interval boundaries rather than just
activity identity. Overall accuracy across all types is 0.38.*

### Figure 2 — Activity confusion matrix

![Activity confusion matrix](outputs/figures/figure2_confusion_matrix.png)

*Figure 2: Confusion matrix for the headline (`full`) recognition model on
held-out users. Confusion is concentrated within two natural groups —
stationary (lying down / sitting / standing) and moving (walking / running
/ bicycling) — with little confusion across the two. Running (F1 0.12) and
standing (F1 0.21) are the weakest classes.*

### Figure 3 — Accuracy versus strictness

![Accuracy versus strictness](outputs/figures/figure3_accuracy_vs_strictness.png)

*Figure 3: How duration accuracy (numeric tolerance) and grounding accuracy
(IoU threshold) change as the correctness criterion is relaxed. Grounding
accuracy rises from 0.37 to over 0.60 as the required timestamp overlap is
loosened, showing many "incorrect" grounded answers are close rather than
far off.*

### Figure 4 — Accuracy versus overhead

![Accuracy versus overhead](outputs/figures/figure4_accuracy_vs_overhead.png)

*Figure 4: Accuracy vs. on-disk model size for the three recognition-model
variants. The `compressed` model is ~300x smaller than `full` (0.05 MB vs.
16.55 MB) and ~85x faster per query, for a loss of only ~4 accuracy points
(0.461 vs. 0.499) — the strongest efficiency result in this project.*

### Figure 5a — Robustness to sensor noise

![Robustness to noise](outputs/figures/figure5a_robustness_noise.png)

*Figure 5a: QA accuracy on a fixed question set as random noise is added to
each feature. Accuracy drops from 0.49 to ~0.42-0.44 once any noise is
present, then stays roughly flat as noise increases further.*

### Figure 5b — Robustness to dropped samples

![Robustness to dropout](outputs/figures/figure5b_robustness_dropout.png)

*Figure 5b: QA accuracy as a fraction of feature values is randomly dropped
(set to NaN, then imputed). Accuracy degrades gradually and predictably,
from 0.49 at no dropout to ~0.40 at 50% of features dropped.*
